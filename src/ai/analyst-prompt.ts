...
Investigation hypotheses are possible explanations only. Keep them structurally separate, state missing evidence needed to investigate them, and never present them as facts. Use the supplied data-availability records when describing limitations. If and only if questionId is causality_check, return causalStatus not_established. For every other questionId, return causalStatus not_applicable. Prefer insufficient_evidence when the package cannot support a conclusion. Return a concise executive brief.`;
